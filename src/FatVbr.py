...
class FatVbr:
    ...
    def __init__(self, file, offset):
        try:
            ...
            self.posLastSector  = 0
            self.posFirstReservedSector  = 0
            self.posLastReservedSector  = 0
            ...
            self.invalidVbr = True

    def isInvalid(self):
        return self.invalidVbr

    def hasWarnings(self):
        return self.unhandledWarning

    def getWarningsAsString(self):
        warningsAsString = ""
        if (self.hasWarnings()):
            warningsAsString+="\n*** WARNING ***"
            warningsAsString+="\nThere are settings enabled which this parser does not support"
            warningsAsString+="\n" + self.unhandledWarningMessage
            warningsAsString+="\n*** WARNING ***\n"
    ...
    def printHeader(self):
        ...
        if self.isInvalid():
            print("\n*** ERROR ***");
            print(" Could not read the VBR")
            print("*** ERROR ***\n");
        print(self.getWarningsAsString())
